...
void DriveSystem()
{
  ...
  JOY_RX = map(ps4.Stick(LX), 0, 255, -100, 100);
  JOY_RX = JOY_RX * (abs(JOY_RX) > deadzone);
  JOY_LY = map(ps4.Stick(LY), 0, 255, -100, 100);
  JOY_LY = JOY_LY * (abs(JOY_LY) > deadzone);

  JOY_LX = map(ps4.Stick(RX), 0, 255, -100, 100);
  JOY_LX = JOY_LX * (abs(JOY_LX) > deadzone);

  lf_val = constrain(JOY_LY + JOY_RX + JOY_LX, -range, range);
  rf_val = constrain(JOY_LY - JOY_RX - JOY_LX, -range, range);
  lb_val = constrain(JOY_LY - JOY_RX + JOY_LX, -range, range);
  rb_val = constrain(JOY_LY + JOY_RX - JOY_LX, -range, range); 
